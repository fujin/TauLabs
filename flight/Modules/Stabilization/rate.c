...
//! Private variables
static float rate_integral[MAX_AXES];
...
{
  if(reinit) {
    rate_integral[axis] = 0;
  }

  rate_integral[axis] = bound_sym(rate_integral[axis], settings->ManualRate[axis]);
...
{
  return stabilization_common_pirocomp(z_gyro, dT, rate_integral);
}
